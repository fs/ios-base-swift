...
@available(iOS 13.0, *)
class DefaultGraphQLAPIClient: GraphQLAPIClient {

    // MARK: - GraphQLAPIClient Properties

    static let shared: GraphQLAPIClient = DefaultGraphQLAPIClient(accessManager: DefaultManagersModule().accessManager)
    ...
    private lazy var client: ApolloClient = {
        ...
        let interceptorProvider = NetworkInterceptorProvider(accessManager: self.accessManager, store: store, isUpdateToken: false)
